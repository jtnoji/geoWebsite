...
/**
 * The no-guarantees pull-quote (mockup `.pull`): red chip label (the page's
 * ONE red chip), 2px ink top rule, 21px/500 quote with bold ink opener and
 * one accent phrase, mono footer with the black NO GUARANTEES chip. Always
 * on a light ground — never a dark block. Copy renders verbatim from
 * HONESTY_COPY parts.
 */
export default function HonestyBlock({ withLabel = true }: { withLabel?: boolean }) {
  ...
  return (
    <aside className="max-w-[680px]">
      {withLabel && <Chip red>{heading}</Chip>}
      <div className="mt-3.5 border-t-2 border-ink pt-5">
        <p className="text-[21px] font-medium leading-[1.45] tracking-[-0.02em] text-ink-soft">
          ...
          {parts.mid}
          <em className="not-italic font-bold text-accent">{parts.accent}</em>
          {parts.rest}
        </p>
